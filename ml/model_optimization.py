...
def create_model_with_hp(model_type, input_shape, hp):
    ...
    optimizer = Adam(
        learning_rate=hp["learning_rate"], beta_1=hp["beta_1"], beta_2=hp["beta_2"]
    )
...
def perform_hyperparameter_optimization(X_train_scaled, y_train, model_type):
    ...
    def create_model(
        hidden_units, dropout_rate, activation, l2_reg, learning_rate, beta_1, beta_2
    ):
        hp = {
        ...
        return create_model_with_hp(model_type, X_train_scaled.shape[1], hp)
    ...
    try:
        ...
        logging.info(
            f"Best {model_type} model parameters: {random_search_result.best_params_}"
        )
        logging.info(
            f"Best {model_type} model score: {random_search_result.best_score_:.4f}"
        )
        ...
        history = default_model.fit(
            X_train_scaled, y_train, batch_size=32, epochs=50, verbose=0
        )
