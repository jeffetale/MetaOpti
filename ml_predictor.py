...
class MLPredictor:
    ...
    def extract_features(self, rates_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(rates_frame) < 50:  # Adjust to fit rolling window
            logging.warning(
                f"Insufficient data points for feature extraction: {len(rates_frame)}"
            )
        ...
        rates_frame["Bollinger_Band_Width"] = self._calculate_bollinger_band_width(
            rates_frame
        )
        ...
        rates_frame["price_change_volatility"] = (
            rates_frame["price_change_1"].rolling(window=10).std()
        )

        # Relative Performance
        rates_frame["relative_strength"] = (
            rates_frame["close"] / rates_frame["close"].rolling(window=50).mean()
        )

        # Drop NaN values
        rates_frame.dropna(inplace=True)

        # If no data remains after dropping NaNs, return empty DataFrame
        if len(rates_frame) == 0:
            logging.warning("No valid data points after feature extraction")
            return pd.DataFrame(columns=self.features)
